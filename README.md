...
### ✨ 主要特性
  ...
  * 👆 **TouchBoost 触摸响应提频** - 基于独立 epoll 线程监听 `/dev/input/event*` 触摸事件，操作时秒级提升底频，松手指数级平滑衰减。
  * 🔀 **CPUSet 核心绑定管理** - 随性能模式与屏幕状态动态控制 cgroup 核心掩码，前台独占高性能核心，后台严格限制于小核。
  * 💤 **CPU 静止下潜 (Idle Dive)** - 轻负载与静止状态下放宽 C-State 限制并自适应切换 idle governor，进一步降低待机与日用功耗。
  * 🌡️ **温度感知调控** - 实时读取 CPU 温度传感器，超过阈值时自动降频保护。
  * 🌙 **智能息屏节电** - 息屏时自动进入 Doze 模式，强制限制性能上限与 CPUSet 绑定，极致省电。
  * 📱 **yumi Bridge Android 控制端 App** - 基于 Android 14 (Target SDK 34) 的 Edge-to-Edge iOS 26 冰雪白毛玻璃沉浸式控制中心，支持实时状态同步、模式切换与 5 级日志分级过滤。
  * 🌐 **轻量 WebUI** - 同时支持嵌入式网页端管理，通过浏览器或 KernelSU 页面管理调度配置。
...
yumi 采用 **Monitor + Scheduler** 双线程架构，结合独立的 TouchBoost 监听线程，通过 `mpsc` 事件通道解耦数据采集与调度决策：
...
│                                                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────┐  ┌──────────────┐ │
│  │ app_detect   │  │ fps_monitor  │  │ cpu_monitor │  │ TouchListener│ │
│  │ (Cgroup检测)  │  │ (eBPF uprobe)│  │(eBPF trace)│  │ (epoll input)│ │
│  └──────┬───────┘  └──────┬───────┘  └─────┬──────┘  └──────┬───────┘ │
│         │                 │                 │               │          │
│         │     ┌───────────┴─────────────────┘               │          │
...
│                                                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐│
│  │ FAS 控制器    │  │ CLG 负载调速 │  │ CPUSet Manager│  │ Idle Dive   ││
│  │ (游戏场景)    │  │ (日常场景)    │  │ (绑核策略)   │  │ (静止下潜)  ││
│  └──────────────┘  └──────────────┘  └──────────────┘  └─────────────┘│
│               ▼                                                        │
│     sysfs 频率 / cpuset / cpuidle 写入 (FastWriter)                    │
└────────────────────────────────────────────────────────────────────────┘
```
...
| 模式 | 图标 | 特点 | 适用场景 |
| :--- | :--- | :--- | :--- |
| **省电 (Powersave)** | 🔋 | 最大化续航，CLG 低性能上限，CPUSet 严格限制小核。 | 待机、轻度使用、阅读。 |
| **均衡 (Balance)** | ⚖️ | CLG 自适应调频，性能与功耗的最佳平衡点。 | 日常使用、社交应用。 |
| **性能 (Performance)** | ⚡ | CLG 高响应配置，优先性能，前台核心解锁。 | 大型应用、轻度游戏。 |
| **极速 (Fast)** | 🚀 | CLG 最大性能释放，全大核绑定。 | 重度任务、性能测试。 |
| **FAS (帧感知调度)** | 🎯 | PID 控制器实时分析帧时间，逐帧动态调频，自动档位切换。 | 游戏场景，兼顾流畅与省电。 |

### 辅助模块说明

1. **CLG (CPU Load Governor)**: 在非 FAS 模式下基于 eBPF 利用率实现平滑自适应调频。
2. **CPUSet Manager**: 随模式与亮灭屏同步调整 `top-app`/`foreground`/`background` 绑核，避免后台抢占性能核。
3. **Idle Dive**: 监测全核平均负载，静止时自动放宽 C-State `latency_us` 提升下潜深度。
4. **TouchBoost**: 触摸屏按下时瞬间提升 `scaling_min_freq` 保证滑动跟手性，松手后阶梯式恢复。
5. **Doze 模式**: 息屏时自动启用，锁死 CPU 频率与 CPUSet 绑定上限，极致省电。

...
pub async fn start_cpu_loop(tx: Sender<DaemonEvent>) -> Result<(), anyhow::Error> {
    ...
    let core_idle_map: PerCpuArray<_, u64> = PerCpuArray::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("CORE_IDLE_TIME")
            .ok_or_else(|| anyhow::anyhow!("CORE_IDLE_TIME not found"))?,
    )?;
    let core_busy_map: PerCpuArray<_, u64> = PerCpuArray::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("CORE_BUSY_TIME")
            .ok_or_else(|| anyhow::anyhow!("CORE_BUSY_TIME not found"))?,
    )?;
    let core_last_time_map: PerCpuArray<_, u64> = PerCpuArray::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("CORE_LAST_TIME")
            .ok_or_else(|| anyhow::anyhow!("CORE_LAST_TIME not found"))?,
    )?;
    let core_current_tid_map: PerCpuArray<_, u32> = PerCpuArray::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("CORE_CURRENT_TID")
            .ok_or_else(|| anyhow::anyhow!("CORE_CURRENT_TID not found"))?,
    )?;
    let thread_run_map: BpfHashMap<_, u32, u64> = BpfHashMap::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("THREAD_RUN_TIME")
            .ok_or_else(|| anyhow::anyhow!("THREAD_RUN_TIME not found"))?,
    )?;

    // TGID 级聚合运行时间 map
    let tgid_run_map: BpfHashMap<_, u32, u64> = BpfHashMap::try_from(
        unsafe { &mut *bpf_ptr }
            .map_mut("TGID_RUN_TIME")
            .ok_or_else(|| anyhow::anyhow!("TGID_RUN_TIME not found"))?,
    )?;
    ...
    let core_current_tgid_map: PerCpuArray<_, u32> = PerCpuArray::try_from(
        unsafe { &mut *bpf_ptr }
            ...
            .ok_or_else(|| anyhow::anyhow!("CORE_CURRENT_TGID not found"))?,
    )?;

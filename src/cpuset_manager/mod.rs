...
// ════════════════════════════════════════════════════════════════
//  CPUSet 管理器 — 动态调整进程的 CPU 核心绑定
//
//  通过 /dev/cpuset 或 /sys/fs/cgroup/cpuset 控制各 cgroup 组
//  可使用的 CPU 核心，实现前台绑大核、后台绑小核的省电调度。
// ════════════════════════════════════════════════════════════════
...
impl CpuSetPolicy {
    /// 根据组名返回对应的 cpuset 值
    fn value_for_group(&self, group: &str) -> Option<&String> {
        match group {

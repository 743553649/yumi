...
fn sd_enter_thresh() -> f32 {
    0.05
}
fn sd_enter_ticks() -> u32 {
    30
}
fn sd_exit_thresh() -> f32 {
    0.15
}
fn sd_exit_ticks() -> u32 {
    15
}
fn sd_exit_boost() -> u32 {
    5
}
fn sd_perf_ceil() -> f32 {
    0.30
}
fn sd_smoothing_up() -> f32 {
    0.05
}
...
impl StillDiveConfig {
    pub fn normalize(&mut self) {
        if !self.enter_threshold.is_finite() {
            self.enter_threshold = sd_enter_thresh();
        }
        if !self.exit_threshold.is_finite() {
            self.exit_threshold = sd_exit_thresh();
        }
        if !self.perf_ceil.is_finite() {
            self.perf_ceil = sd_perf_ceil();
        }
        if !self.smoothing_up.is_finite() {
            self.smoothing_up = sd_smoothing_up();
        }
        ...
        if self.enter_ticks == 0 {
            self.enter_ticks = sd_enter_ticks();
        }
        if self.exit_ticks == 0 {
            self.exit_ticks = sd_exit_ticks();
        }
        if self.exit_boost_ticks > 100 {
            self.exit_boost_ticks = 100;
        }
        if self.exit_boost_ticks == 0 {
            self.exit_boost_ticks = sd_exit_boost();
        }
    }
}

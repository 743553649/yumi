...
| `(已移除)` | 📁 | **Android 控制端 App 源码**。基于 Java/XML + Jetpack Compose 混合架构实现的 iOS 26 冰雪白毛玻璃 Target SDK 34 (已移除) 控制端应用。 |
| `(已移除)app/src/main/java/com/yumi/bridge/ui/` | 📁 | **原生 UI 逻辑**。包含 `GlassCardView` (真实毛玻璃卡片) 与 `CpuCircleProgressView` (CPU 多核图表)。 |
| `(已移除)app/src/main/java/com/yumi/bridge/ui/compose/` | 📁 | **Jetpack Compose 视图**。包含 `LiquidControlCenter.kt` (流体控制中心)、`LiquidCpuDashboard.kt` (CPU 仪表盘) 与 `LiquidMeshBackground.kt` (流体背景)。 |
| `(已移除)build_apk.sh` | 📄 | Termux 手机端轻量级原生 Java APK 编译构建脚本。 |
| `(已移除)build_compose_apk.py` | 📄 | Termux 手机端 Jetpack Compose AAR/JAR 提取、资源合并与全功能 APK 构建 Python 脚本。 |
...
### 📁 `src/.rs` — TCP Loopback 双向通信服务端

| 文件 | 作用 |
|:---|:---|
| `.rs` | **IPC 文本行服务端**。在 127.0.0.1:14567 端口监听 TCP 连接，解析 `ping`/`get_mode`/`set_mode` 文本行指令，异步向 IPC 主循环派发 `DaemonEvent::ModeChange` 事件。 |

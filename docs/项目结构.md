...
| `build.rs` | 📄 | **构建脚本**。编译时自动安装 `bpf-linker`，交叉编译 `yumi-ebpf` crate 为 BPF 目标 (`bpfel-unknown-none`) 并嵌入主二进制。支持 `YUMI_SKIP_EBPF=1` 跳过 eBPF 编译进行快速语法检查。 |
| `README.md` | 📄 | 中文项目文档。详述架构、FAS 原理、CLG/TouchBoost/CPUSet/IdleDive 方案、配置参数、安装与故障排除。 |
| `README.en.md` | 📄 | 英文版项目文档。 |
| `LICENSE` | 📄 | GNU GPL v3.0 开源许可证。 |
| `.gitignore` | 📄 | Git 忽略规则 (target/, .idea/, *.zip, output/ 等)。 |
| `android-app/` | 📁 | **Android 控制端 App 源码**。基于 Java/XML + Jetpack Compose 混合架构实现的 iOS 26 冰雪白毛玻璃 Target SDK 34 Edge-to-Edge 控制端应用。 |
| `android-app/app/src/main/java/com/yumi/bridge/ui/` | 📁 | **原生 UI 逻辑**。包含 `GlassCardView` (真实毛玻璃卡片) 与 `CpuCircleProgressView` (CPU 多核图表)。 |
| `android-app/app/src/main/java/com/yumi/bridge/ui/compose/` | 📁 | **Jetpack Compose 视图**。包含 `LiquidControlCenter.kt` (流体控制中心)、`LiquidCpuDashboard.kt` (CPU 仪表盘) 与 `LiquidMeshBackground.kt` (流体背景)。 |
| `android-app/build_apk.sh` | 📄 | Termux 手机端轻量级原生 Java APK 编译构建脚本。 |
| `android-app/build_compose_apk.py` | 📄 | Termux 手机端 Jetpack Compose AAR/JAR 提取、资源合并与全功能 APK 构建 Python 脚本。 |
| `docs/` | 📁 | **项目相关设计方案与指南文档目录**（含项目结构、编译/测试指南、UI毛玻璃重构架构规范等）。 |
| `docs/项目结构.md` | 📄 | **本文件** — 项目结构地图。 |
...
### 📁 `src/cpuset_manager/` — CPUSet 绑核策略管理器

| 文件 | 作用 |
|:---|:---|
| `mod.rs` | **CPUSet 管理器**。管理 top-app、foreground、system-background、background 及 dex2oat 核心分配。根据性能模式（powersave/balance/performance/fast/doze）动态改写 `/dev/cpuset` cgroup 节点的 cpus 属性。 |

### 📁 `src/idle_dive/` — CPU 静止下潜控制器

| 文件 | 作用 |
|:---|:---|
| `mod.rs` | **Idle Dive 控制器**。在低负载与静止状态下放宽 `/sys/devices/system/cpu/cpuidle/` 节点的 latency_us 限制，自适应切换底层 cpuidle governor 提升 C-State 深度。 |

### 📁 `src/touch_boost/` — TouchBoost 触摸响应提频

| 文件 | 作用 |
|:---|:---|
| `mod.rs` | **TouchBoost 控制器**。基于 epoll 独立线程直接监听 `/dev/input/event*` 触摸节点（ABS_MT_TRACKING_ID），捕获触摸按下后提升指定 CPU 簇 scaling_min_freq，松手后按指数衰减恢复。 |

### 📁 `src/ipc_server.rs` — TCP Loopback 双向通信服务端

| 文件 | 作用 |
|:---|:---|
| `ipc_server.rs` | **IPC 文本行服务端**。在 127.0.0.1:14567 端口监听 TCP 连接，解析 `ping`/`get_mode`/`set_mode` 文本行指令，异步向 IPC 主循环派发 `DaemonEvent::ModeChange` 事件。 |
...
| `customize.sh` | **安装脚本**。自动检测系统语言 (中文/英文)，输出欢迎信息；安装前自动备份已有用户配置 (`rules.yaml` 及 `config.yaml`)，解压后还原用户配置。 |
...
| `config/config.yaml` | **核心主配置**。Meta (日志等级/语言)、功能开关、I/O 优化、CPU Idle、4 种性能模式的 CLG 参数。 |
| `config/cpuset.yaml` | **CPUSet 绑核配置**。定义 5 种模式下各个 cgroup 组允许使用的 CPU 掩码列表。 |
| `config/idle_dive.yaml` | **静止下潜配置**。定义 Dive / Exit 负载阈值、延迟时长及 C-State latency 参数。 |
| `config/touch_boost.yaml` | **触摸提频配置**。定义各 CPU Policy 的提频目标频率、持续时间与衰减比例。 |
| `config/i18n/en.ftl` | **Rust 守护进程英文日志翻译**。Fluent 格式，覆盖 Monitor/Scheduler/FAS/CLG/CPUSet/IdleDive/TouchBoost 模块。 |

...
pub fn run() -> Result<Vec<TestResult>> {
    ...
    results.push(test_check_clg_config()?);

    Ok(results)
}

fn test_check_clg_config() -> Result<TestResult> {
    ...
    let path = if std::path::Path::new(config_path).exists() {
        ...
        return Ok(TestResult {
            module: "clg".to_string(),
            name: "check_clg_config".to_string(),
    ...
    let status = if content.contains("cpu_load_governor") || content.contains("clg") {
        TestStatus::Pass
    } else {
        TestStatus::Skip("CLG config not found (may be disabled)".to_string())
    };

    ...
    return () => {
      if (isObj(props.value)) {
        ...
        if (isPerAppProfileNode) {
          slots.value = () => h('div', { onClick: (e: Event) => e.stopPropagation() }, [
            h(Icon, {
              name: 'delete-o', color: '#EF4444', size: '18',
      ...
      return h(Cell, {
        title: props.name, center: true, isLink: typeof props.value !== 'boolean',
        ...
        value: () => {
          ...
          if (Array.isArray(props.value)) {
            displayVal = props.path.endsWith('cluster_profiles')
              ? `[${props.value.map((v: any) => v.capacity_weight ?? '?').join(', ')}]`
              : `[${props.value.join(', ')}]`;
          } else {
        ...
        'right-icon': () => typeof props.value === 'boolean' ? h(Switch, {
          modelValue: props.value, size: '20px',
          'onUpdate:modelValue': (newVal: boolean) => { setDeepValue(currentData.value, props.path, newVal); saveConfig(); }
        }) : null
      });
    };
  }
...
<template>
  <div class="config-editor">
    ...
    <div class="tab-wrap">
      <van-tabs v-model:active="activeTab" type="card" animated @change="loadData" color="rgba(59,130,246,0.6)" background="transparent">
    ...
    <van-dialog v-model:show="showEditDialog" :title="t('edit')" show-cancel-button @confirm="confirmEdit" class="glass-modal">
...
.tab-wrap :deep(.van-tab) {
  color: var(--text-secondary) !important;
}
.tab-wrap :deep(.van-tab--active) {
  color: var(--text-primary) !important;
}
.tab-wrap :deep(.van-tabs__nav--card) {
  border-color: var(--glass-border) !important;
}
.tab-wrap :deep(.van-tab--card) {
...
:deep(.van-collapse-item .van-collapse-item) {
  border: none;
  border-radius: 0;
  margin-bottom: 0;
